...
class ShellEmulator(tk.Tk):
    ...
    def _setup_ui(self):                                      #метод для создания и настройки пользовательского интерфейса
        ...
        self.title(f"Эмулятор - [{username}@{hostname}]")     #Устанавливает заголовок окна в формате "Эмулятор - [user@hostname]"
        self.output_area = scrolledtext.ScrolledText(         #Область вывода
            self, state='disabled', wrap='word',
            bg='black', fg='white', font=('Consolas', 12)
        )
        self.output_area.pack(expand=True, fill='both', padx=5, pady=5) #размещение текстового поля

        input_frame = tk.Frame(self, bg='black')              #создание фрейма для группировки элемента ввода
        input_frame.pack(fill='x', padx=5, pady=(0, 5))

        self.prompt_label = tk.Label(                         # Приглашение командной строки
            input_frame,
        ...
        self.input_entry = tk.Entry(                              # Поле ввода команды
        ...
        self.input_entry.focus_set()                                            #Устанавливает фокус ввода на поле ввода команд
        self.input_entry.bind("<Return>", self._on_enter)                       #Привязывает обработчик события нажатия клавиши Enter.

    def _get_prompt(self):                                 #Метод для формирования строки приглашения командной строки
        username = getpass.getuser()
        hostname = platform.node()
        dir_name = os.path.basename(self.current_dir)      #Извлекает имя текущей рабочей директории из полного пути
    ...
    def _display_output(self, text):
        ...
        self.output_area.see(tk.END)                                      #Прокручивает область вывода к самому концу текста

    def _expand_variables(self, text):                         #Раскрытие переменных окружения
        result = text
    ...
    def _on_enter(self, event=None):                          # Обработчик нажатия клавиши Enter в поле ввода команд
        command_text = self.input_entry.get().strip()
        self.input_entry.delete(0, tk.END)

        if not command_text:                                  #Команда не была введена
            return

        self._display_output(self._get_prompt() + command_text) #выводится команда с приглашением

        expanded_command = self._expand_variables(command_text) #раскрываются переменные окружения

        parts = expanded_command.split()                        #разбиваем на команду и аргументы
        if not parts:
            return
        command = parts[0]
        args = parts[1:] if len(parts) > 1 else []
        # Выполнение команд-заглушек
        if command == "exit":
            self.quit()
        elif command == "ls":
